...
def sol7():
    ...
    print(
        "Best position is: {0} with fuel cost: {1}".format(
            best_position, int(best_fuel_cost)
        )
    )
    ...
    arrow = dict(facecolor="black", width=0.5, headwidth=4, shrink=0.1)
    ...
    plt.annotate(
        "Best position: {0}".format(best_position),
        xy=(best_position, best_fuel_cost),
        xytext=[best_position / 2.2, best_fuel_cost * 4],
        arrowprops=arrow,
    )
    plt.legend()
    plt.show()
