...
with open("../data/advent_7.txt", "r") as text:
...
def parentbag(childbag):  # function to find what bags can hold the childbag
    for parent in bags_dict:  # iterate through the different bags
        ...
        if childbag in content:  # if the requested bag can be in parent bag
            ...
            bagset.add(
                parent
            )  # add bag to set of bags allowed to (eventually) hold childbag
    return
...
for bag in bags:  # populate the dictionary
    bag = (
        bag.replace(" bags", "").replace(" bag", "").replace(".", "")
    )  # Remove text not required
    bag = bag.split(
        " contain "
    )  # split each entry into parent and child bag - note the spaces
    ...
    if content[0] == "no other":
        ...
        for child in content:  # iterate through the bags inside the parent bag
            ...
            if bagname in children_counting:  # TRUE if bag has been counted already
                ...
                children_counting[bagname] = (
                    number  # if the bag has not been counted already, add the number of it
                )
            for i in range(number):  # iterate through the amount of bags
                add_child(
                    bagname
                )  # go through each of the child bags inside the parent bag and get their content
        return
